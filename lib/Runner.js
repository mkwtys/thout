  ...
  _nextTest() {
    if (this._pendingTests.length === 0) {
      ...
      if (this._reporter) {
        ...
        if (result.pass > 0) {
          this._reporter.log(chalk.green(`${result.pass} passed`));
        }
        if (result.fail > 0) {
          this._reporter.log(chalk.red(`${result.fail} failed`));
        }
        if (result.skip > 0) {
          this._reporter.log(chalk.cyan(`${result.skip} pending`));
        }
      }
      return;
    }
    ...
    test.bind();
    if (this._reporter) {
      this._reporter.log(test.getFilename());
    }
    ...
    const done = () => {
      ...
      if (this._reporter) {
        if (test.isPass()) {
          this._reporter.log(`${chalk.green(symbol.success)} ${test.getTitle()}`);
        } else if (test.isFail()) {
          this._reporter.log(`${chalk.red(symbol.error)} ${test.getTitle()}`);
        } else if (test.isSkip()) {
          this._reporter.log(`${chalk.cyan(symbol.pending)} ${test.getTitle()}`);
        }
      }
